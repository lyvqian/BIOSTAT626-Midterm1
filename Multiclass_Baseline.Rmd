...
multiclass.roc(knn_pred,as.numeric(test$multi_activity)) # check auc
```
...
multiclass.roc(nb_pred,as.numeric(test$multi_activity)) # check auc
```
...
This is chosen as the final algorithm.
...
multiclass.roc(svm.pred2,as.numeric(test$multi_activity))  # check auc
```
...
multiclass.roc(rf_pred,as.numeric(test$multi_activity))
```
...
multiclass.roc(pre_xgb,label4test)
```
...
multiclass.roc(knn_pred_cor,as.numeric(test$multi_activity))
```
...
multiclass.roc(svm.pred3,as.numeric(test$multi_activity))
```
...
multiclass.roc(svm.pred4,as.numeric(test$multi_activity))
```
...
multiclass.roc(pre_xgb_cor,label4test_cor)
```
...
multiclass.roc(pred.rf.select,as.numeric(test.rf.select$multi_activity))
```
...
multiclass.roc(pre_xgb_rf,label4test_rf)
```
